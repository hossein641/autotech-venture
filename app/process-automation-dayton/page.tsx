import { Metadata } from 'next';
import Header from '@/components/layout/Header';
import Footer from '@/components/layout/Footer';

export const metadata: Metadata = {
  title: 'Process Automation Dayton | Business Automation | AutoTech Venture',
  description: 'Process automation services in Dayton, Ohio. Streamline operations with intelligent automation solutions by AutoTech Venture.',
  alternates: {
    canonical: 'https://www.atechv.com/process-automation-dayton',
  },
};

export default function ProcessAutomationPage() {
  return (
    <main>
      <Header />
      <section className="bg-gradient-to-br from-green-50 to-blue-100 py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            <h1 className="text-4xl md:text-5xl font-bold text-gray-900 mb-6">
              Process Automation in Dayton
            </h1>
            <p className="text-xl text-gray-600 max-w-3xl mx-auto">
              Streamline your business operations with intelligent automation solutions.
            </p>
          </div>
        </div>
      </section>
      <Footer />
    </main>
  );
}
